...
class AoeComponents_Validation_Html extends AoeComponents_Validation_Base {
	...
	protected $validationUrl = 'http://validator.w3.org/check';
    ...
    public function isValid($html, $aOptions = array()) {
        ...
        $position = isset($aOptions['document_section_position']) ? $aOptions['document_section_position'] : self::POSITION_BODY;
        ...
        if ( self::HTTP_IDENTIFIER == mb_substr( $html, 0, mb_strlen( self::HTTP_IDENTIFIER ) ) && is_null( $doctypeOverride ) ) {
            ...
            if ( self::HTML_CHUNK == $section ) {
                if ( self::POSITION_HEAD == $position ) {
                    $html = <<< HTML
